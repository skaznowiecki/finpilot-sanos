        ...
        <CardContent>
            ...
            <div v-else class="space-y-3">
                <BankAccountCard
                    ...
                    :account="account"
                    :saving="saving"
                    :can-delete="canDeleteAccount(account.id)"
                    @update="(data) => handleUpdate(account.id, index, data)"
                    @delete="() => handleDelete(account.id, index)"
                    @primary-change="(isPrimary) => handlePrimaryChange(account.id, index, isPrimary)"
                />
            </div>
        </CardContent>
    </Card>
...
import NewBankAccountForm from './NewBankAccountForm.vue'
...
const handleUpdate = (accountId: string, index: number, data: UpdateBankAccountRequest) => {
    emit('update', accountId, data)
}

const handleDelete = (accountId: string, index: number) => {
    emit('delete', accountId)
}

const handlePrimaryChange = (accountId: string, index: number, isPrimary: boolean) => {
    if (isPrimary) {
